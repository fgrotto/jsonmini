...
char *json_type_map[] = {
    [json_number] = "json_number", [json_string] = "json_string", [json_boolean] = "json_boolean",
    [json_null] = "json_null",	   [json_object] = "json_object", [json_array] = "json_array",
};
...
static struct json_value array(struct json *json);
static struct json_value object(struct json *json);
static struct json_value parse(struct json *json);
...
static void skip_whitespace(struct json *json)
{
	while (!json->is_eof(json) &&
	       (json->cur(json) == ' ' || json->cur(json) == '\t' || json->cur(json) == '\n'))
	{
		json->pos++;
	}
}
...
/**
 * Parse an atom (number, boolean, null, string, array, or object) from the
 * input.
 */
static struct json_value atom(struct json *json)
{
...
static struct json_value array(struct json *json)
{
	...
	while (!json->is_eof(json) && json->cur(json) != ']')
	{
		...
		json_value.values =
		    realloc(json_value.values, sizeof(struct json_value) * (json_value.length + 1));
...
static struct json_value object(struct json *json)
{
	...
	while (!json->is_eof(json) && json->cur(json) != '}')
	{
		...
		json_value.values =
		    realloc(json_value.values, sizeof(struct json_value) * (json_value.length + 1));
		json_value.values[json_value.length] = member;
		json_value.object_keys =
		    realloc(json_value.object_keys, sizeof(char *) * (json_value.length + 1));
...
/**
 * Free the memory associated with a JSON value.
 */
